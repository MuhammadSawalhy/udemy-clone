...
function renderCarousel() {
  ...
  for (let slide = 0; slide < slidesCount; slide++) {
    ...
    const maxCount = Math.min(slide * cardsCount + cardsCount, courses.length);
    for (let i = slide * cardsCount; i < maxCount; i++) {
...
function courseToMarkup(course) {
  ...
  return `
    <a class="course" href="#">
      ...
      <span class="text-small-lite">${course.instructors
        .map((i) => i.name)
        .join()}</span>
